...
const appeal_service_1 = require("../../src/services/appeal-service");
const buildAppealRow = (overrides = {}) => ({
    id: 10,
    userId: 42,
...
const buildMessageRow = (overrides = {}) => ({
    id: 5,
    appealId: 10,
...
const createService = () => {
    ...
    const withTransaction = jest.fn(async (handler) => handler({}));
    ...
    const service = new appeal_service_1.AppealService(databaseService, userService, operationLogService);
    return {
        service,
        query,
        queryWithClient,
...
describe('AppealService', () => {
    ...
    it('submits appeals and stores the initial message', async () => {
        ...
        ctx.userService.getInternalUserId.mockResolvedValue(42);
        ...
        expect(ctx.operationLogService.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'submit_user_appeal' }), expect.any(Object));
    });
    it('rejects submissions when pending appeals exceed limit', async () => {
        const ctx = createService();
        ctx.userService.getInternalUserId.mockResolvedValue(42);
        ctx.queryWithClient.mockResolvedValueOnce([{ count: '3' }]);
        await expect(ctx.service.submitAppeal({ synapseUserId: '@demo:matrix.local', reason: 'Need help' }, '@demo:matrix.local')).rejects.toMatchObject({ statusCode: 400 });
    });
    it('decides appeals and records the decision', async () => {
        ...
        ctx.queryWithClient
            ...
            // fetch messages
            .mockResolvedValueOnce([messageRow]);
        const detail = await ctx.service.decideAppeal(pendingRow.id, { status: 'accepted', responseMessage: 'Welcome back' }, '1');
        expect(detail.status).toBe('accepted');
        expect(detail.messages[0].author).toBe('admin:1');
        expect(ctx.operationLogService.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'appeal_decision' }), expect.any(Object));
    });
});
//# sourceMappingURL=appeal-service.test.js.map